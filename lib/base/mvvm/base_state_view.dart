...
import 'eventbus/base_event_bus.dart';
import 'loading/indicator.dart';
import 'page_state_view_model.dart';
import 'router/page_router.dart';
import 'statepage/mf_page_empty.dart';
import 'statepage/mf_page_error.dart';
import 'statepage/page_loading.dart';
import 'statistics/statistics_data.dart';
import 'vm_event.dart';

///切换状态栏 模式：light or dark
abstract class BaseStateView<T extends BaseViewModel> extends BaseState
  ...
  Widget build(BuildContext context) {
    ...
    return MultiProvider(
        ...
        child: AnnotatedRegion(child: createPage(context, buildView(context)), value: getSystemUiOverlayStyle() == null ?
        SystemUiOverlayStyle.dark : getSystemUiOverlayStyle()));
  }
  ...
  List<Widget> createPageView(Widget contentView, BuildContext context) {
    ...
    List<VMEvent> event = registerVMEvent(context);
    // 事件监听
    if (event != null) {
      widgets.addAll(event);
    }
    widgets.add(contentView);

    if (showMultiPageState()) {
      // 转圈提示
  ...
  Widget createLoadingView() {
    return Selector<PageStateViewModel, Tuple2<bool, String>>(
        builder: (context, isShow, _) {
          ...
          if (isShow.item1) {
            children.add(MFActivityIndicator(
              animating: true,
              radius: 20,
            ));
          }
  ...
  List<ChangeNotifierProvider> createProviders(BuildContext context) {
    ...
    var baseProvider = createProvider<T>(viewModel);
  ...
  /// 注册事件监听
  @override
  List<VMEvent> registerVMEvent(BuildContext context) {
    return null;
  }
  ...
  /// 点击返回按钮
  void onBack<T extends Object>({T param}) {
    MvvmX.pagePop(param);
  }
